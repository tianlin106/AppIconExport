...
/**
  ...
  - */
- (NSString *) replaceURLParam:(NSString *) param value:(NSString *)value{
    ...
    for (NSTextCheckingResult *match in matches) {
        ...
        NSString *tagValue = [self substringWithRange:range];  // 分组2所对应的串
        if (![value isEqualToString:tagValue]) {
            url = [self stringByReplacingCharactersInRange:range withString:value];
        }
    }
    return url;
}
